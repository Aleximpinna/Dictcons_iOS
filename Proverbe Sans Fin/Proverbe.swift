//
//  Proverbe.swift
//  Proverbe Sans Fin
//
//  Created by Alexandre Impinna on 02/08/2017.
//  Copyright © 2017 Alexandre Impinna. All rights reserved.
//

import Foundation

class Proverbe {
    
    var proverbe: [String] = ["Il ne faut pas vendre la peau de l'ours",
                              "On n'apprend pas aux vieux singes",
                              "Il faut prendre le taureau",
                              "Il ne faut pas se moquer des chiens",
                              "Il ne faut pas péter plus haut",
                              "Il vaut mieux avoir affaire à Dieu",
                              "Il ne faut pas prendre les enfants",
                              "Les bons comptes",
                              "Loin des yeux",
                              "L'amour nait d'un regard",
                              "On ne peut avoir le beurre",
                              "La vengeance est un plat",
                              "Mieux vaut sagesse",
                              "Si jeunesse savait",
                              "Les voyages forment"]
    
}

